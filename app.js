...
function create(csvFile) {
  ...
  d3.csv(csvFile, d => {
    ...
    x.domain(tmpData.map(d => { return d.Country; }));
    y.domain([0, d3.max(tmpData, d => { return d.Wealth; })]);
    ...
    bars
      ...
      .on('mouseover', (d) => {
        div.transition()
          .duration(50)
          .style("opacity", 1);
        const num = `${d.Country}: \n${((d.Wealth / 360474) * 100).toFixed(3).toString()  }%`;
        div.html(num)
    ...
    svg.append('text')
      ...
      .text('Top 51 Countries by Total Wealth (2019)');
